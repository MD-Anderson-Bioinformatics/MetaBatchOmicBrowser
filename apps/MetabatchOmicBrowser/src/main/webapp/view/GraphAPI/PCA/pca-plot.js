...
function PcaPlot(model, node, legendNode, params)
{
	...
	// used to add padding to left of main diagram to add room for DSC
	var dscPadding = 0;
	// ### await all !
	...
	var greyscale = params.greyscale;
	...
	// increased magic number of height
	...
	function makePcaPlot(pcaValues, batchInfo, pcaAnnotations)
	{
		...
		var nest = d3.nest()
				...
				{
					return d.Type;
				})			// Type = Run|Diagram|Component
				.key(function (d)
				{
					return d.SubType;
				});		// SubType really only important for Component
	...
	function performAnnotationSubstitution(annotations)
	...
	function makePlot()
	{
		...
		{
			dscPadding = extraInfoWidth;
			batchOMeter = riskGradient({parent: svg.node(), x: 0, y: 20, greyscale: greyscale})();
			var temp = diagramAnnotations[0];							// ### kludgy
			if (temp['Annotation'] === 'DSC')
			
				temp = temp['Value'];		// ### else?
	...
	function redraw()
	{
		...
		if (Math.abs(ticks[1] - ticks[0]) > Math.pow(10, 4))
		{		// 10^4 is arbitrary
			fx = d3.format(".0e");
		} else
		{
		...
		ticks = y.ticks(10);
		if (Math.abs(ticks[1] - ticks[0]) > Math.exp(10, 4))
		{		// 10^4 is arbitrary
			fy = d3.format(".0e");
		} else
		{
		...
		function resetBoundList(sel, data, makeLink)
		{
			if (notUN(data))
			{
				...
				x.selectAll('.annot-label')
						...
						.attr('target', '_blank');
				x.selectAll('.annot-val').text(function (d)
				{
					return d[1];
				});
			}
	...
	function redrawData()
	{
		...
		function ffx(val)
		{
			return fixedFormatter(x(val));
		}
		function ffy(val)
		{
			return fixedFormatter(y(val));
		}
		...
		function drawScatter(layer)
		{
			...
			batches.enter().append("svg:g")
					.attr("class", function ()
					{
						return isHiliteLayer ? "batch-hilite hiliter batch" : "batch";
					})
					.attr("visibility", function ()
					{
						return isHiliteLayer ? "hidden" : "visible";
					});	// ???
			...
			batches
					...
					{
						return getBatchColor(d);
					})
					.attr("stroke-width", function (d, i)
					{
						return getBatchStroke(d);
					})
					.attr("visibility", function (d)
					{
						return hiliteVisible(d);
					});
			...
			if (installedOptions.includeCentroids)
			{
				...
				lines.enter().append("svg:line")
						.attr("class", function ()
						{
							return isHiliteLayer ? "hiliter ptline" : "ptline";
						})
			...
			points.enter().append("svg:path")
					.attr("class", function ()
					{
						return isHiliteLayer ? "hiliter dot" : "dot";
					})
			...
			points
					.attr("id", function (d)
					{
						return "" + d.batch.batchID + "-" + d[pointIdField];
					})
					.attr("transform", function (d)
					{
						return "translate(" + ffx(d[xDimension]) + "," + ffy(d[yDimension]) + ")";
					})
					.attr("d", function (d)
					{
						return d.batch.symbol;
					})
		...
		if (installedOptions.includeCentroids)
		{
			...
			centroids
					...
					.attr("transform", function (d)
					{
						return "translate(" + ffx(d.centroid[xDimension]) + "," + ffy(d.centroid[yDimension]) + ")";
					})
					.attr("d", function (d)
					{
						return d.symbol;
	...
	function getTrendLines(d)
	{
		...
		trendBatches.sort(function (a, b)
		{
	...
	function killHiliteOverlay()
	{
		...
		centroidLayer.selectAll("path.centroid").style("fill", function (d)
		{
			return d.color;
		});
	}
	...
	function _makeLegend(div)
	{
		...
		legendBatches.sort(function (a, b)
		{
			return (a.batchID < b.batchID ? -1 : (a.batchID > b.batchID ? 1 : 0));
		});
...
//				.on("mouseout", legendRectMouseout)		// ??? ###
		...
		batches.enter().append("svg:g")
				...
				{
					return "translate(10," + (((i + 1) * 20) + spaceBelowTitle) + ")";
				})
		...
		labels.enter().append("svg:text")
				...
				.text(function (d)
				{
					return d.batchID + " (" + d.points.length + ")";
				});
	...
	function findAxisLabelWidth(labels, el)
	{
		...
		var maxWidth = d3.max(labels, function (label)
		{
			return myTextWidth(textnode, label)[0];
		});
		return maxWidth;
	}
	...
	{
		var textnode = el.node();
		var maxHeight = d3.max(labels, function (label)
		{
			return myTextWidth(textnode, label)[1];
		});
		return maxHeight;
	}
	...
	{
		xDomain[0] = d3.min(batchData, function (batch)
		{
			return d3.min(batch.points, function (pt)
			{
				return pt[xDimension];
			});
		});
		xDomain[1] = d3.max(batchData, function (batch)
		{
			return d3.max(batch.points, function (pt)
			{
				return pt[xDimension];
			});
		});
		...
		yDomain[0] = d3.min(batchData, function (batch)
		{
			return d3.min(batch.points, function (pt)
			{
				return pt[yDimension];
			});
		});
		yDomain[1] = d3.max(batchData, function (batch)
		{
			return d3.max(batch.points, function (pt)
			{
				return pt[yDimension];
	...
	function _resizePlot(wNew, hNew)
	{
		...
		plotPanel
				.attr("x", plotGeometry.leftAxisWidth + dscPadding)
				.attr("y", plotGeometry.titleHeight)
				.attr("width", plotGeometry.plotWidth)
				.attr("height", plotGeometry.plotHeight);

		hAxisPanel
				.attr("x", plotGeometry.leftAxisWidth - hLabelMargin + dscPadding)
		...
		titleElem
				.attr("x", (plotGeometry.plotWidth / 2) + dscPadding)
				.attr("y", plotGeometry.titleHeight - 2);

		var titleArrayWithLineNums = [];
		if (titleArray !== null)
		{
			titleArray.forEach(function (line, ix)
			{
				titleArrayWithLineNums.push([ix, line]);
			});
		}
	...
	function _getGraphVariables()
	{
		return plotDataFields.slice(0);
...
PcaPlot.BasicModel = function (pcaValues, batchInfo, annotations, thePointIdColumn)
{
	...
	model.getPcaValues = function ()
	{
		return pcaValues;
	};
	model.getBatchInfo = function ()
	{
		return batchInfo;
	};
	model.getPcaAnnotations = function ()
	{
		return annotations;
	};
	model.getPointId = function ()
	{
		return thePointIdColumn;
	};
	return model;
};
