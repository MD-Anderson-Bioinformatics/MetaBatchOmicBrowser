...
HierClustPlot.uniqueSuffix = 1;


function HierClustPlot(model, domNode, params)
{
	var uniqueSuffix = (HierClustPlot.uniqueSuffix++).toString();
	var d3plusTitle = null;
	var svg = null,
			dendroPanel = null,
			barsPanel = null,
			yAxis = null,
	...
	var plotAnnotation = params.plotAnnotation;
	var plotTitle = params.plotTitle;

	if (showDetailFunc === undefined)
	{
		showDetailFunc = null;
	}
	...
	function dataReady(model, domNode)
	{
		...
		var justOrders = orders.map(function (ord)
		{
			return ord.Order - 1;
		});
	...
	function hiliteSimilarSamples(d)
	{
		...
		leaves.forEach(function (sample)
		{
			if (sample.covars[d.covar] === d.value)
	...
	function makePlot(domNode)
	{
		...
		if ((plotTitle !== undefined) && (plotTitle !== null))
		{
			
			//create the title svg with the generated id
			svg.append("g")
				.attr("id", "plottitle"+uniqueSuffix)
				.attr("width", '100%')
				.attr("height", plotGeometry.titleHeight);
			const data = [{text: plotTitle}];
			//d3.select("#BarPlottingTitle").text(theTitle);
			d3plusTitle = new d3plus.TextBox()
				.select("#plottitle" + uniqueSuffix)
				.data(data)
				.fontSize(10)
				.textAnchor("middle")
				.fontResize(true)
				.width(plotGeometry.leftAxisWidth + plotGeometry.plotWidth + plotGeometry.rightAxisWidth)
				.height(plotGeometry.titleHeight)
				.render();
			$("#plottitle").addClass('plottitle' + uniqueSuffix);
			
                        
		}

		dendroPanel = svg.append("svg:svg").attr("class", "DendroPanel");
	...
	function _resizePlot(wNew, hNew)
	{
		...
		console.log("hc::resizePlot plotGeometry.plotWidth=" + plotGeometry.plotWidth);
		//when resizing just gets the new width and rerenders the textBox
		if (notUN(d3plusTitle))
		{
			d3plusTitle.width(plotGeometry.leftAxisWidth + plotGeometry.plotWidth + plotGeometry.rightAxisWidth).render();
		}
		//
		//svg.select("text.plottitle")
		//		.attr("x", plotGeometry.leftAxisWidth + (plotGeometry.plotWidth / 2))
		//		.attr("y", plotGeometry.titleHeight - 10)
		//		.attr("width", plotGeometry.plotWidth)
		//		.attr("height", plotGeometry.titleHeight);
	...
	function _makeLegend(covar, div)
	{
		...
		symbols.enter().append("svg:rect")
				...
				.style("fill", function (d, i)
				{
					return color(colorIndex(covar, d.key));
				});
